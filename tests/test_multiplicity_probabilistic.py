"""
Probabilistic multiplicity analysis tests.

These tests call ProbabilisticMultiplicityAnalyzer directly via the
run_probabilistic_analysis helper from conftest, bypassing semantic and static
analysis. They verify warning presence, correctness of warning fields, and
monotonicity properties of the probability estimates.
"""

import pytest

from conftest import run_probabilistic_analysis, warnings_for


# No warnings on clearly satisfiable specs


def test_no_warning_weight_one_count_within_bounds(multiplicityLang_path):
...
def test_no_warning_unbounded_max_with_high_weight(multiplicityLang_path):
...
def test_no_warning_zero_min_weight_zero(multiplicityLang_path):
    """weight=0.0 on a field with mult_min=0 (unbounded lower) is trivially
    satisfied."""
    spec = """
let servers = Server(1);
...
def test_no_warning_large_right_set_satisfies_high_lower_bound(multiplicityLang_path):
...
def test_no_warning_subsystem_within_bounds(multiplicityLang_path):
...
def test_no_warning_many_left_assets_one_right_asset_bounded_forward(
    multiplicityLang_path,
):
    """Many Databases connected to a single Server at weight 1.0: each
    Database receives exactly one hostedOn edge, satisfying multiplicity=1.
    The Server's reverse databases field is unbounded so also fine."""
    spec = """
...
def test_warning_weight_zero_with_positive_lower_bound(multiplicityLang_path):
...
def test_warning_very_low_weight_high_lower_bound(multiplicityLang_path):
...
"""
    warnings = run_probabilistic_analysis(spec, multiplicityLang_path, threshold=0.9)
    ws = warnings_for(warnings, 'Server', 'monitoredBy')
    assert len(ws) == 1
    assert ws[0].global_probability < 0.9


def test_warning_right_set_too_small_for_lower_bound(multiplicityLang_path):
    """Even weight=1.0 cannot satisfy mult_min=5 if right set has only 2
    assets; should warn with probability 0."""
    spec = """
let servers_a = Server(1);
...
def test_warning_subsystem_low_weight_positive_lower_bound(multiplicityLang_path):
...
def test_warning_many_left_assets_high_weight_bounded_reverse(multiplicityLang_path):
    """Many Credentials connected to a single Client at high weight risks
    violating the reverse multiplicity on 'credential' (mult=1 on Client)."""
    spec = """
...
"""
    warnings = run_probabilistic_analysis(spec, multiplicityLang_path, threshold=0.5)
    ws = warnings_for(warnings, 'Client', 'credential')
    assert len(ws) == 1
    assert ws[0].global_probability < 0.5


# Warning field correctness


def test_warning_contains_correct_multiplicity_info(multiplicityLang_path):
...
"""
    warnings = run_probabilistic_analysis(spec, multiplicityLang_path, threshold=0.001)
    ws = warnings_for(warnings, 'Server', 'installedSoftware')
    assert len(ws) == 1
...
def test_same_spec_warns_at_high_threshold_not_low(multiplicityLang_path):
    """A borderline spec should warn at a strict threshold but not a lenient
    one."""
    spec = """
let servers = Server(1);
let software = Software(2);
...
"""
    ws_strict = warnings_for(
        run_probabilistic_analysis(spec, multiplicityLang_path, threshold=0.99),
        'Server',
        'installedSoftware',
    )
    ws_lenient = warnings_for(
        run_probabilistic_analysis(spec, multiplicityLang_path, threshold=0.01),
        'Server',
        'installedSoftware',
    )
...
def test_threshold_zero_never_warns(multiplicityLang_path):
...
"""
    warnings = run_probabilistic_analysis(spec, multiplicityLang_path, threshold=0.0)
    assert warnings == []


def test_threshold_one_warns_unless_certain(multiplicityLang_path):
...
"""
    warnings = run_probabilistic_analysis(spec, multiplicityLang_path, threshold=1.0)
    ws = warnings_for(warnings, 'Server', 'installedSoftware')
    assert len(ws) == 1
...
def test_higher_weight_gives_higher_global_probability(multiplicityLang_path):
...
"""
        ws = warnings_for(
            run_probabilistic_analysis(spec, multiplicityLang_path, threshold=0.0),
            'Server',
            'installedSoftware',
        )
        return ws[0].global_probability if ws else 1.0

    assert get_global_p(0.1) <= get_global_p(0.5) <= get_global_p(0.9)


def test_larger_right_set_increases_per_asset_probability(multiplicityLang_path):
    """For a lower-bound constraint, more assets in the right set with the
    same weight should increase per_asset_probability."""

    def get_per_asset_p(n: int) -> float:
        spec = f"""
let servers = Server(1);
...
"""
    ...
    assert get_per_asset_p(1) <= get_per_asset_p(3) <= get_per_asset_p(6)


def test_more_assets_of_checked_type_decreases_global_probability(
    multiplicityLang_path,
):
...
"""
    ...
    assert get_global_p(1) >= get_global_p(5) >= get_global_p(10)


# Multi-rule accumulation


def test_two_rules_accumulate_higher_probability_than_one(multiplicityLang_path):
...
"""
    ws_two = warnings_for(
        run_probabilistic_analysis(spec_two, multiplicityLang_path, threshold=0.0),
        'Server',
        'installedSoftware',
    )
    ws_one = warnings_for(
        run_probabilistic_analysis(spec_one, multiplicityLang_path, threshold=0.0),
        'Server',
        'installedSoftware',
    )
...
def test_accumulated_rules_equivalent_to_single_larger_rule(multiplicityLang_path):
...
def test_subsystem_asset_counts_multiplied_correctly(multiplicityLang_path):
...
"""
    ws_sub = warnings_for(
        run_probabilistic_analysis(spec_sub, multiplicityLang_path, threshold=0.0),
        'Server',
        'installedSoftware',
    )
    ws_flat = warnings_for(
        run_probabilistic_analysis(spec_flat, multiplicityLang_path, threshold=0.0),
        'Server',
        'installedSoftware',
    )
...
def test_nested_subsystem_asset_counts_propagate_correctly(multiplicityLang_path):
...
"""
    ws = warnings_for(
        run_probabilistic_analysis(spec, multiplicityLang_path, threshold=1.0),
        'Server',
        'installedSoftware',
    )
    # 3 * 2 = 6 Server assets total
    assert len(ws) == 1
...
def test_no_warning_param_constant_within_bounds(multiplicityLang_path):
    """A param = constant used as asset count within bounds should not warn."""
    spec = """
param n = 2;
...
def test_no_warning_param_distribution_expected_within_bounds(multiplicityLang_path):
...
def test_warning_param_zero_constant_with_positive_lower_bound(multiplicityLang_path):
...
def test_warning_param_distribution_low_expected_count(multiplicityLang_path):
...
"""
    ws = warnings_for(
        run_probabilistic_analysis(spec, multiplicityLang_path, threshold=0.9),
        'Server',
        'monitoredBy',
    )
    assert len(ws) == 1
...
def test_param_constant_scales_global_probability_via_asset_count(
    multiplicityLang_path,
):
...
def test_param_arithmetic_expected_value_is_correct(multiplicityLang_path):
...
"""
    ws_param = warnings_for(
        run_probabilistic_analysis(spec_param, multiplicityLang_path, threshold=0.0),
        'Server',
        'installedSoftware',
    )
    ws_lit = warnings_for(
        run_probabilistic_analysis(spec_literal, multiplicityLang_path, threshold=0.0),
        'Server',
        'installedSoftware',
    )
    p_param = ws_param[0].per_asset_probability if ws_param else 1.0
    p_lit = ws_lit[0].per_asset_probability if ws_lit else 1.0
    assert p_param == pytest.approx(p_lit, abs=1e-6)


def test_param_distribution_expected_value_matches_literal(multiplicityLang_path):
...
def test_param_used_in_subsystem_scales_global_probability(multiplicityLang_path):
